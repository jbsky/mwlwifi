			       ...
			       u32 changed)
{
	...
	if (priv->debug_config) wiphy_debug(hw->wiphy, "change: 0x%x\n", changed);
	...
	if (changed & IEEE80211_CONF_CHANGE_POWER) {
		...
		ieee80211_queue_delayed_work(hw, &priv->txpower, SYSADPT_DELAY_TXPOWER);
